        ...
        <form action="${pageContext.request.contextPath}/rating" method="post" id="ratingForm">
            ...
            <div style="display: flex; justify-content: center; gap: 24px; align-items: center;">
                ...
                <button type="submit" class="bb-btn bb-btn--primary" style="padding: 16px 60px; font-size: 1.1rem;">
                    <%= existingRating != null ? "Update Experience" : "Submit Experience" %>
                </button>
                <div style="height: 1px; flex: 1; background: var(--bb-border);"></div>
            </div>
        </form>
...
<script>
    document.getElementById('ratingForm').addEventListener('submit', function(e) {
        var food = document.querySelector('input[name="foodRating"]:checked');
        var staff = document.querySelector('input[name="staffRating"]:checked');
        var ambience = document.querySelector('input[name="ambienceRating"]:checked');

        if (!food || !staff || !ambience) {
            var msg = "Please provide a rating for the following:";
            if (!food) msg += "\n- Food Quality";
            if (!staff) msg += "\n- Staff Behavior";
            if (!ambience) msg += "\n- Atmosphere";
